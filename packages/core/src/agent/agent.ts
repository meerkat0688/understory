...
export async function runQuery(
  ...
  const chain = modelFallbackChain(options);
  ...
  for (let i = 0; i < chain.length; i += 1) {
    ...
    let modelChain: string[] = [];
    try {
      const resolved = await resolveAgentModel({ model: modelId }, "query");
      modelChain = resolved.modelChain;
      const result = await generateText({
        model: resolved.model,
        system: buildSystemPrompt(ctx),
      ...
      const trace = recorder.finalize("query", question, errorMessage(error), "failed", modelChain);
      await traceStore(kb).save(trace);
      throw error;
    }
  }
...
export async function runMutation(
  ...
  if (chain.length === 0) {
    return runMutationOnce(kb, instruction, options, ctx);
  }

  let lastError: unknown;
  for (let i = 0; i < chain.length; i += 1) {
    const modelId = chain[i];
    const outcome = await runMutationOnce(kb, instruction, { model: modelId }, ctx);
    if (outcome.ok) return outcome;
    if (outcome.status === "partial") return outcome;

    lastError = new Error(outcome.error);
    const next = chain[i + 1];
    if (
      next &&
      isContentFilterError(lastError) &&
      canRetryMutationAfterError(0, false)
    ) {
      console.warn(
...
async function runMutationOnce(
  kb: KnowledgeBase,
  instruction: string,
  options: AgentOptions,
  ctx: Awaited<ReturnType<typeof promptContext>>
): Promise<MutationOutcome> {
  const recorder = new TraceRecorder();
