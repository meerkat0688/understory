...
/** ~5 MB JSON body — above the global 4 MB cap, below default CHAT_MAX_REQUEST_BYTES. */
function oversizedJsonBody(): string {
  return JSON.stringify({ messages: "x".repeat(5 * 1024 * 1024) });
}

describe("chat vs global JSON body limits", () => {
  it("accepts a 5 MB POST /api/chat body (chat limit, not global 4 MB)", async () => {
    ...
    try {
      ...
      // Parsed by chat's larger limit; invalid messages → 400, not 413.
  ...
  it("accepts a 5 MB POST /api/chat/ body (trailing slash)", async () => {
    const { server, port } = await listen(createBodyLimitApp());
    try {
      const res = await fetch(`http://127.0.0.1:${port}/api/chat/`, {
        method: "POST",
  ...
  it("rejects a 5 MB POST /mcp body under the global 4 MB cap", async () => {
    const { server, port } = await listen(createBodyLimitApp());
    try {
